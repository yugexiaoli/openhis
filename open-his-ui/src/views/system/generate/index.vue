<template>
  <div class="app-container code-gen">
    <el-form ref="genForm" class="gen-form" :model="clientParam" size="mini" label-width="150px">
      ...
      <el-form-item v-show="showTable" label="包名（package）" required="true">
        <el-input v-model="clientParam.packageName" placeholder="必填：默认com.twofish" show-word-limit maxlength="100" />
      </el-form-item>
    </el-form>
...
<script>
import { datasourceadd, datasourceupdate, datasourcetest, datasourcelist, templatelist, datasourcedbtype, datasourcetable, datasourcedel } from '@/api/system/generator/template'
export default {
  data() {
    return {
      showTable: false,
  ...
  methods: {
    ...
    onDataSourceChange(datasourceConfigId) {
      ...
      datasourcetable(`${datasourceConfigId}`).then(resp => {
        this.showTable = true
    ...
    onDatasourceTest() {
      this.$refs.datasourceForm.validate((valid) => {
        if (valid) {
          datasourcetest(this.datasourceFormData).then(resp => {
            this.tip('连接成功')
          })
        }
      })
        ...
        if (valid) {
          datasourcetest(this.datasourceFormData).then(resp => {
            if (this.datasourceFormData.id) {
              datasourceupdate(this.datasourceFormData).then(resp => {
                location.reload()
              })
            } else {
              datasourceadd(this.datasourceFormData).then(resp => {
                this.tip('添加成功')
                this.loadDataSource()
